...
TreeNode *leftClonedToNormal(TreeNode* node){
	if(node == NULL){
		return NULL;
	}
	TreeNode *ln = leftClonedToNormal(node->left);
	TreeNode *rn = leftClonedToNormal(node->right);
	if(!ln) return node;
	if(ln->val == node->val){
	node->left = ln->left;
	node->right = rn;
...
int main(){
	...
	TreeNode* root1 = leftClonedToNormal(root);
	displayBT(root1);
	return 0;
}